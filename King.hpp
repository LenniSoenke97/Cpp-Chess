...
public:
  ...
  /*
   * Description: canMakeMove returns whether a king on a certain source field can make
   ...
   *              field, is max 1 field away and that no other figures are in the way.
   * Input:       ChessField* source: the chess field that the king you want to move is on
   *              ChessField* destination: the chess field you want to move the king to
  ...
  bool castling(ChessField* source, ChessField* destination, ChessField* board[8][8]) const;
