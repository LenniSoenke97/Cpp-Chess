...
class Pawn : public Piece
{
   ...
   * Input:       ChessField* source: the chess field that the pawn you want to 
   *              move is on
...
public:
  ...
  /*
   * Description: canMakeMove returns whether a pawn on a certain source field 
   *              can make a move to a given destination field. The function 
   *              checks whether the  destination field is within moving 
   *              distance from the source field, that no other figures are in
   *              the way, the destination field is straight ahead of the pawn
   *              or the field is diagonal from the pawn and an enemy figure is
   *              on this field.
   * Input:       ChessField* source: the chess field that the pawn you want to
   *              move is on
   *              ChessField* destination: the chess field you want to move the
  ...
  /*
   * Description: this function marks sets the pawn's has_moved attribute to 
   *              true and changes the maximum movable distance to 1 (since 
   *              only pawns who have not moved yet can move two fields forward)
   * Input:       none
   * Output:      void
   */
