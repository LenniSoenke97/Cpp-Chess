...
public:
  ...
  /*
   * Queen Constructor
   * Description: constructor of queen, sets the colour of queen and calls 
  ...
  /*
   * Description: canMakeMove returns whether a queen on a certain source field
   *              can make a move to a given destination field. The function
   *              checks whether the destination field is diagonal, horizontal
   *              or vertical from the source field and that no other figures
   *              are in the way.
   * Input:       ChessField* source: the chess field that the queen you want 
   *              to move is on
